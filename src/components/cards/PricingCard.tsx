...
const PricingCard = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 right-0 w-24 h-24 bg-blue-500/10 rounded-full -mt-12 -mr-12 blur-xl transition-all duration-500 ${
          isHovered ? "scale-125" : "scale-100"
        }`}
      ></div>
      <div
        className={`absolute bottom-0 left-0 w-32 h-32 bg-purple-500/10 rounded-full -mb-16 -ml-16 blur-xl transition-all duration-500 ${
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-lg font-medium text-muted-foreground mt-6">
          {planName}
        </h3>
        ...
        <div className="mt-8 space-y-4">
          {features.map((feature, index) => (
            <div
              key={index}
              className="flex gap-3 items-center group/item transition-all duration-300 hover:translate-x-1"
            >
        ...
        >
          ...
          <span className="absolute right-4 opacity-0 -translate-x-2 transition-all duration-300 group-hover/btn:opacity-100 group-hover/btn:translate-x-0">
            →
          </span>
        </Button>
